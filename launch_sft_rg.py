#!/usr/bin/env python
"""
Load the Response Generator (RG) SFT dataset uploaded by generate_sft_datasets.py,
...
def main():
    parser = argparse.ArgumentParser(description="Launch RG SFT on Together AI")
    parser.add_argument(
        "--dataset",
        default=HF_REPO_RG,
        help=f"HuggingFace dataset repo (default: {HF_REPO_RG})",
    )
    parser.add_argument(
        "--split",
        default="train",
    ...
    parser.add_argument(
        "--suffix",
        default="multiwoz-rg-sft",
        help="Suffix for the fine-tuned model name (default: rg-sft)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    print(f"Loading RG dataset: {args.dataset} (split={args.split})")
